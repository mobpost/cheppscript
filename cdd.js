/* cheppcodecolor ver 1.21 */
function cheppCodeColor() {
...
for (j = 0; j < k; j++) {
    ...
    l = x.length;
    for (i = 0; i < l; i++) {
        x[i].innerHTML = cheppCodeColorize(x[i].innerHTML, modes[j]);
    }
}
}
function cheppCodeColorize(x, lang) {
  ...
  function htmlMode(txt) {
    ...
    php = new extract(rest, "&lt;\\?php", "?&gt;", phpMode, "CHEPPPHPPOS");
    rest = php.rest;
    comment = new extract(rest, "&lt;!--", "--&gt;", commentMode, "CHEPPHTMLCOMMENTPOS");
    rest = comment.rest;
    ...
    for (i = 0; i < comment.arr.length; i++) {
        rest = rest.replace("CHEPPHTMLCOMMENTPOS", comment.arr[i]);
    }
    for (i = 0; i < php.arr.length; i++) {
        rest = rest.replace("CHEPPPHPPOS", php.arr[i]);
    }
    return rest;
  }
  ...
  function cssMode(txt) {
    ...
    comment = new extract(rest, /\/\*/, "*/", commentMode, "CHEPPCSSCOMMENTPOS");
    rest = comment.rest;
    while (rest.search("{") > -1) {
      s = rest.search("{");
    ...
    rest = rest.replace(/}/g, "<span style=color:" + cssdelimitercolor + ">}</span>");
    for (i = 0; i < comment.arr.length; i++) {
        rest = rest.replace("CHEPPCSSCOMMENTPOS", comment.arr[i]);
    }
  ...
  function jsMode(txt) {
    ...
    for (i = 0; i < esc.length; i++) {
      rest = rest.replace("CHEPPJSESCAPE", esc[i]);
    }
